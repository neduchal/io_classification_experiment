...
for t in types:
    for hs in hist_settings:
        dataset_name = dataset_name_base + "_" + t + "_" + str(hs)
        print("Creating dataset " + dataset_name)
        output_directory = os.path.join(output_path, dataset_name)
        if not os.path.exists(output_directory):
            os.makedirs(output_directory)
        print("VAL")
        val_data_x, val_data_y, val_data_y_io = common.process_hist_io_multiscale(val_names, classes_val, classes_io, classes_nums, fname_begin_val, True, t, hs)
        common.save_to_h5(os.path.join(output_directory, 'val.h5'), val_data_x, val_data_y, val_data_y_io)
        print("TRAIN")
        train_data_x, train_data_y, train_data_y_io = common.process_hist_io_multiscale(train_names, classes, classes_io, classes_nums, fname_begin_train, False, t, hs)
        common.save_to_h5(os.path.join(output_directory, 'train.h5'), train_data_x, train_data_y, train_data_y_io)
        print("TEST")
        test_data_x, test_data_y, test_data_y_io = common.process_hist_io_multiscale(test_names, classes, classes_io, classes_nums, fname_begin_train, False, t, hs)
